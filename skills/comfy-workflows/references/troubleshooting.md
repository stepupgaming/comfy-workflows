...
| Install missing custom nodes               | `inspect --json`; show `comfy node install <id>`; install only if asked | `comfy node install` unprompted; clone `repository`      |
...
| Unknown custom class                       | Snapshot `/object_info` + codegen; declare Registry id                  | Guess from GitHub name                                   |
