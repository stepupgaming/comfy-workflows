...
Coding agents: [AGENTS.md](./AGENTS.md) (this repo) · skills `skills/comfy-workflows/` (graphs) and `skills/comfy-custom-nodes/` (codegen / setup), also inside the installed package · `cwf agent install` copies both to `.agents/skills/` · [llms.txt](https://stepupgaming.github.io/comfy-workflows/llms.txt)
...
Core `@stepupgaming/comfy-workflows` **0.2.16**. Canonical publication is GitHub (Packages + Release tarballs). First-party examples `@stepupgaming/comfy-workflow-t2i` and `…-hires` follow the same policy. npmjs is a convenience mirror. Workflow **format** is host-agnostic.
